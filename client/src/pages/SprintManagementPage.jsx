...
export default function SprintManagementPage() {
	...
	const fetchSprintBurndown = useWorkspaceStore((state) => state.fetchSprintBurndown);
	...
	if (sprintsLoading && sprints.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center min-h-[500px] text-slate-400">
				...
				<span className="font-mono-ui text-xs">Loading sprint backlog and cycles...</span>
			</div>
		);
	}
			...
			<main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
				...
				<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pb-6 border-b border-slate-200/80 dark:border-slate-800">
					<div>
						...
						<p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-mono-ui">
							Scrum planning, sprint commitments, velocity metrics, and burndown charts
						</p>
					</div>
					...
					<button
						...
						className="flex items-center gap-1.5 self-start sm:self-auto px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold shadow-xs transition-colors"
					>
				...
				{activeSprint ? (
					<section className="my-8 rounded-2xl border border-amber-300 dark:border-amber-900/80 bg-white dark:bg-slate-900 shadow-md overflow-hidden">
						<div className="p-6 bg-gradient-to-r from-amber-500/10 via-orange-500/5 to-transparent border-b border-slate-100 dark:border-slate-800">
							<div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
								<div>
									<div className="flex items-center gap-2">
										<span className="inline-flex items-center gap-1 bg-amber-500 text-white font-mono-ui text-[10px] font-bold uppercase px-2.5 py-0.5 rounded-full shadow-2xs">
									...
									<div className="flex flex-wrap items-center gap-4 mt-4 text-xs font-mono-ui text-slate-500 dark:text-slate-400">
								...
								<div className="flex items-center gap-2 self-end sm:self-start">
									<button
										...
										className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-xs transition-colors font-mono-ui"
									>
							...
							{/* Progress Meter */}
							<div className="mt-6 font-mono-ui">
								<div className="flex items-center justify-between text-xs mb-1.5">
									<span className="font-semibold text-slate-700 dark:text-slate-300">
										Sprint Completion Progress
									</span>
									<span className="font-bold text-slate-900 dark:text-slate-100">
										{activeSprint.completed_tasks || 0}/{activeSprint.total_tasks || 0} tasks (
						...
						<div className="p-6">
							...
							{allTasks.filter((t) => t.sprint_id === activeSprint.id).length === 0 ? (
								<p className="text-xs text-slate-400 font-mono-ui py-4">
									No tasks assigned to active sprint. Drag or assign tasks below.
								</p>
							) : (
					...
					<div className="my-8 rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 text-center">
						...
						<p className="text-xs text-slate-400 font-mono-ui mt-1 max-w-sm mx-auto">
							Start a planned sprint below or create a new sprint to begin an Agile cycle.
						</p>
					</div>
				)}
					...
					<section className="my-8 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-6 shadow-2xs">
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
							<div>
								...
								<p className="text-xs text-slate-400 font-mono-ui mt-0.5">
							...
							{sprints.length > 0 && (
								<select
									...
									className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 text-xs font-mono-ui text-slate-700 dark:text-slate-200 outline-none"
								>
									{sprints.map((s) => (
						...
						{/* Chart Visualization */}
						<div className="h-48 flex items-end gap-2 border-b border-l border-slate-200 dark:border-slate-800 pt-6 px-4 pb-2 font-mono-ui">
								...
								return (
									...
									>
										...
										<div className="pointer-events-none absolute -top-12 hidden group-hover:block bg-slate-900 text-white text-[10px] py-1 px-2 rounded font-bold shadow-lg z-20 whitespace-nowrap">
											...
											{d.actualPoints !== null ? `${d.actualPoints} pts` : "N/A"}
										</div>
									</div>
								);
							})}
						...
						<div className="flex items-center justify-center gap-6 mt-4 text-xs font-mono-ui">
							<div className="flex items-center gap-2">
									...
									>
										<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
											...
											<button
												...
												className="flex items-center gap-1.5 self-start sm:self-auto px-3.5 py-1.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold shadow-xs transition-colors font-mono-ui"
											>
												...
												{sprintTasks.map((task) => (
													...
													>
														...
														<button
															...
															className="text-[11px] text-rose-500 font-mono-ui hover:underline"
														>
															Remove
														</button>
													</div>
				...
				<section className="my-8 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-6 shadow-2xs">
					<div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
						...
						<span className="font-mono-ui text-xs font-bold bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-full text-slate-600 dark:text-slate-300">
						...
						<div className="divide-y divide-slate-100 dark:divide-slate-800 mt-2">
							{backlogTasks.map((task) => (
								...
								>
									...
									<div className="flex items-center gap-2 self-end sm:self-auto font-mono-ui text-xs">
										{/* Sprint Assignment Dropdown */}
										<select
											value=""
											onClick={(e) => e.stopPropagation()}
											onChange={(e) => handleAssignSprint(task.id, e.target.value)}
